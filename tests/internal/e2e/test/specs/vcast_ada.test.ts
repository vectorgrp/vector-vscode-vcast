...
describe("vTypeCheck VS Code Extension", () => {
  ...
  // NOTE: The Reqs2X tests below are DISABLED for now (it.skip). They require
  // the Ada-capable reqs2tests distribution ("autoreq-linux-with-ada"): the
  // stock "autoreq-linux" distribution's code2reqs cannot index Ada
  // environments (fails at "Indexing codebase" with "Translation unit file not
  // found"), and CI currently pulls the stock one. The with-ada distribution
  // indexes Ada envs fine (verified locally on sp1 + sp3), so once CI points at
  // it (e.g. via R2T_RELEASE_URL_LIN) re-enable these by changing it.skip -> it.
