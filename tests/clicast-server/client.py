...
def testCaseTests(clicastPath, enviroPath, testString):
    ...
    # executeTestReport
    print("  Execute Test with Report ...")
    request = vcastDataServerTypes.clientRequest(
        commandType.executeTestReport, clicastPath, enviroPath, testString
    )
    returnData = transmitCommand(request)
    # fix up stuff that changes between runs ...
    returnData = cleanExecutionData(returnData)
    assert compareToExpected("expected-executeTestReport.json", returnData)
    print("     executeTestReport Test Passed")
