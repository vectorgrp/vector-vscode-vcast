...
def check_excel_file_contents(file_path: Path, requirements: list[dict]):
    wb = openpyxl.load_workbook(file_path)
...
def test_main_pre_existing_requirements(monkeypatch, envs_dir, real_requirements_dir):
    ...
    requirements: dict = json.loads(
        (real_requirements_dir / 'requirements.json').read_text()
    )
    ...
    with tempfile.TemporaryDirectory() as tmp_folder:
        ...
        with patch(
            'autoreq.trace_reqs2code.Reqs2CodeMapper.map_reqs_to_code_for_env',
