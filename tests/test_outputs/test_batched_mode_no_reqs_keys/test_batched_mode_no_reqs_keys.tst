...
Verifies that the waiting list supports up to 10 parties by adding a party when the list is at its maximum capacity (index 9), ensuring the party is added at the last slot and the size is incremented appropriately.
TEST.END_NOTES:
...
Verifies that a party can be added to the waiting list by specifying its name, and that the name is correctly stored at the current WaitingListSize index and the size is incremented.
TEST.END_NOTES:
TEST.VALUE:manager.<<GLOBAL>>.WaitingListSize:2
TEST.VALUE:manager.Add_Party_To_Waiting_List.Name:"Johnson"
TEST.EXPECTED:manager.<<GLOBAL>>.WaitingList[2]:"Johnson"
TEST.EXPECTED:manager.<<GLOBAL>>.WaitingListSize:3
TEST.END
...
TEST.NEW
TEST.NAME:Get_Check_Total_Returns_Table_Check_Total-REVIEW-NEEDED
TEST.NOTES:
WARNING: This test case was generated by an AI-based system. Please review it carefully.

Tested Requirement ID: FR22

Tests that Get_Check_Total returns the Check_Total field for a given table by stubbing Get_Table_Record to return a specific Check_Total value. This directly exercises the system's ability to provide the check total for a given table as required by FR22.1.
TEST.END_NOTES:
...
Tests that when the WaitingListIndex is greater than 9, Get_Next_Party_To_Be_Seated resets the index to 0 and returns the name at the start of the list. This ensures the head of the waiting list is reset when reaching the end.
TEST.END_NOTES:
...
This test verifies that after calling Clear_Table, the table's check total is set to 0.0 in the table database, as required by FR21.1. The test sets up a table with a nonzero check total and ensures that Update_Table_Record is called with Check_Total set to 0.
TEST.END_NOTES:
TEST.STUB:uut_prototype_stubs.Get_Table_Record
TEST.STUB:uut_prototype_stubs.Update_Table_Record
...
Tests that placing an order with no entree does not increase the table's check total. The initial check total is set to 15.0, and after placing the order, it should remain 15.0.
TEST.END_NOTES:
TEST.STUB:uut_prototype_stubs.Get_Table_Record
TEST.STUB:uut_prototype_stubs.Update_Table_Record
...
Tests that when an order does not match either the steak/caesar/mixed drink or lobster/green/wine combinations, the system does not set a free dessert (Dessert remains unchanged) per FR27.3.
TEST.END_NOTES:
TEST.VALUE:manager.Add_Included_Dessert.Order:<<malloc 1>>
...
TEST.VALUE:uut_prototype_stubs.Get_Table_Record.return.Wait_Person:"Dan"
TEST.VALUE:uut_prototype_stubs.Get_Table_Record.return.Order[0].Entree:NO_ENTREE
TEST.VALUE:uut_prototype_stubs.Get_Table_Record.return.Order[1].Entree:NO_ENTREE
TEST.VALUE:uut_prototype_stubs.Get_Table_Record.return.Order[2].Entree:NO_ENTREE
TEST.VALUE:uut_prototype_stubs.Get_Table_Record.return.Order[3].Entree:NO_ENTREE
...
Verifies that the system supports all specified entree types by placing orders with each entree and checking the correct entree is set in the table data.
TEST.END_NOTES:
TEST.STUB:uut_prototype_stubs.Get_Table_Record
TEST.STUB:uut_prototype_stubs.Update_Table_Record
TEST.VALUE:manager.Place_Order.Table:2
...
Ensures that after clearing a table, the Update_Table_Record stub is called with all Order fields for all seats set to NO_SOUP, NO_SALAD, NO_ENTREE, NO_DESSERT, and NO_BEVERAGE, as required by FR20.1.
TEST.END_NOTES:
TEST.STUB:uut_prototype_stubs.Get_Table_Record
TEST.STUB:uut_prototype_stubs.Update_Table_Record
...
Ensures that after clearing a table, the Update_Table_Record stub is called with Is_Occupied set to v_false, as required by FR18.1.
TEST.END_NOTES:
TEST.STUB:uut_prototype_stubs.Get_Table_Record
TEST.STUB:uut_prototype_stubs.Update_Table_Record
TEST.VALUE:manager.Clear_Table.Table:2
...
Ensures that after clearing a table, the Update_Table_Record stub is called with Number_In_Party set to 1, as required by FR19.1.
TEST.END_NOTES:
TEST.STUB:uut_prototype_stubs.Get_Table_Record
TEST.STUB:uut_prototype_stubs.Update_Table_Record
...
Tests that placing an order updates the correct seat's order in the table database. The test places an order for seat 2 at table 1 and verifies that Update_Table_Record is called with the new order at the correct seat index.
TEST.END_NOTES:
TEST.STUB:uut_prototype_stubs.Get_Table_Record
TEST.STUB:uut_prototype_stubs.Update_Table_Record
TEST.VALUE:manager.Place_Order.Table:1
...
Tests that placing an order for chicken increases the table's check total by 10.0. The test places a chicken order and verifies the check total is incremented by 10.0 in the updated table data.
TEST.END_NOTES:
TEST.STUB:uut_prototype_stubs.Get_Table_Record
TEST.STUB:uut_prototype_stubs.Update_Table_Record
...
Tests that placing an order for lobster increases the table's check total by 18.0. The test places a lobster order and verifies the check total is incremented by 18.0 in the updated table data.
TEST.END_NOTES:
TEST.STUB:uut_prototype_stubs.Get_Table_Record
TEST.STUB:uut_prototype_stubs.Update_Table_Record
...
Tests that placing an order for steak increases the table's check total by 14.0. The test places a steak order and verifies the check total is incremented by 14.0 in the updated table data.
TEST.END_NOTES:
TEST.STUB:uut_prototype_stubs.Get_Table_Record
TEST.STUB:uut_prototype_stubs.Update_Table_Record
TEST.VALUE:manager.Place_Order.Table:2
