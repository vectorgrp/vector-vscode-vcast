...
class FileUtils:
    ...
    def download_and_extract_archive(
        ...
        """
        Downloads the archive from the given URL having format {archive_type}
        and extracts it to the given {target_path}
        """
        try:
            tmp_files = []
            tmp_file_name = str(
            ...
            if archive_type in ["zip", "tar", "gztar", "bztar", "xztar"]:
                ...
                f_in = gzip.open(tmp_file_name, "rb")
                try:
                    f_out = open(tmp_file_name_ungzipped, "wb")
                    try:
