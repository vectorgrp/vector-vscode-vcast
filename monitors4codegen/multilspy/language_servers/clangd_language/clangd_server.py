...
class ClangdServer(LanguageServer):
    ...
    def setup_runtime_dependencies(
        ...
        assert platform_id.value in ['linux-x64', 'win-x64'], (
        ...
        clangd_executable_path = os.path.join(
            clangdls_dir, 'clangd_18.1.3', 'bin', dependency['binaryName']
        )
    ...
    async def start_server(self) -> AsyncIterator['ClangdServer']:
        ...
        async with super().start_server():
            self.logger.log(
                'Starting clangd-language-server server process', logging.INFO
            )
