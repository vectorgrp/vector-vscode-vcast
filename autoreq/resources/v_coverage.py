...
def get_test_case_by_id(env_path, test_case_id):
    with UnitTestApi(env_path + ".vce") as api:
        ...
        for test_case in test_cases:
            test_id = test_case.unit_display_name + '.' + test_case.function_display_name + '.' + test_case.name
...
def get_aggregate_coverage_info(test_cases):
    ...
    for test in test_cases:
        ...
        covered_statements |= set(map(tuple, test_coverage_data['covered_statement_indices']))
        covered_true_branches |= set(map(tuple, test_coverage_data['covered_true_branches_indices']))
        covered_false_branches |= set(map(tuple, test_coverage_data['covered_false_branches_indices']))
    ...
    coverage_data = {
        "statements": {
            ...
            "percentage": (num_covered_statements / num_total_statements) if num_total_statements > 0 else 0
        },
        "branches": {
            "covered": num_covered_branches,
            "total": num_total_branches,
            "percentage": (num_covered_branches / num_total_branches) if num_total_branches > 0 else 0
        }
    }
...
def main():
    ...
    parser.add_argument("test_case_ids", nargs="+", help="ID (unit.suprogram.test_name) of the test cases.")
