...
class RequirementsManager:
    def __init__(self, requirements_or_path: t.Union[t.List[t.Dict], str]):
        ...
        for req in self._requirements:
            if req['Function'] == 'None':
                logging.warning(f"Requirement {req['ID']} has no function assigned. Discarding.")
                continue
    ...
    def _load_from_csv(self, path: t.Union[str, Path]):
        ...
        with open(path, newline='') as csvfile:
            reader = csv.DictReader(csvfile, skipinitialspace=True, quoting=csv.QUOTE_MINIMAL)
            for row in reader:
                reqs.append(row)
        return reqs
...
class DecomposingRequirementsManager(RequirementsManager):
    ...
    @staticmethod
    async def from_requirements_manager(original_requirements_manager, decomposer=lambda r: [r]):
        """
        ...
        tasks = [process_requirement(req_id) for req_id in original_requirements_manager.requirement_ids]
        ...
        return DecomposingRequirementsManager(original_requirements_manager, sub_requirements)
    ...
    async def from_file(original_requirements_path, decomposer=lambda r: [r]):
        ...
        return await DecomposingRequirementsManager.from_requirements_manager(original_requirements, decomposer)
    ...
    def filter(self, filter_callback):
        ...
        for orig_id, sub_reqs in self.sub_requirements.items():
            filtered_sub_requirements[orig_id] = [req for req in sub_reqs if filter_callback(req)]
            
        return DecomposingRequirementsManager(self.original_requirements, filtered_sub_requirements)
