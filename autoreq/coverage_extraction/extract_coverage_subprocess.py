    ...
    with UnitTestApi(environment_path) as uapi:
        for unit in uapi.Unit.all():
            ...
            for function in unit.functions:
                ...
                statement_coverage_function_mapping[function_name]['num_statements'] = (
                    function.metrics.statements
                )
                statement_coverage_function_mapping[function_name][
                    'statement_coverage'
                ] = function.metrics.aggregate_covered_statements
                statement_coverage_function_mapping[function_name][
                    'percentage_statement_coverage'
                ] = function.metrics.aggregate_covered_statements_pct

                statement_coverage_function_mapping[function_name]['lines'] = []

                function_offset = (
                    function.inst_start_line
                    - function_info[function.name]['start_line']
                )

                statement_coverage_function_mapping[function_name]['offset'] = (
                    function_offset
                )
                statement_coverage_function_mapping[function_name][
                    'source_start_line'
