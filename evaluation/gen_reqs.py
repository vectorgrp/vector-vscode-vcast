...
async def process_environments(env_files, max_n, max_cost, output_file, allow_existing=False):
...
async def main():
    parser = argparse.ArgumentParser(description='Generate requirements from environment files')
    parser.add_argument('--env-list', required=True, help='File containing list of environment files or directory to scan for .env files')
    parser.add_argument('--max-n', type=int, default=100, help='Maximum number of environments to process')
    parser.add_argument('--max-cost', type=int, default=100, help='Maximum total cost')
    parser.add_argument('--output', help='Output file to save processed environment list')
    parser.add_argument('--allow-existing', action='store_true', help='Skip generation if requirements already exist')
