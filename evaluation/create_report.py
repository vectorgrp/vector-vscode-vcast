...
def calculate_aggregated_metrics(results: List[EvaluationResult]) -> Dict[str, Any]:
    ...
    metrics = {
        ...
        'micro_precision': total_verified_tests / total_generated_tests if total_generated_tests > 0 else 0,
        'micro_recall': total_verified_tests / total_requirements if total_requirements > 0 else 0,
        
        # Average metrics (macro-averages)
        'avg_precision': np.mean([result.precision for result in valid_results]) if valid_results else 0,
        'avg_recall': np.mean([result.recall for result in valid_results]) if valid_results else 0,
        'avg_f1_score': np.mean([result.f1_score for result in valid_results]) if valid_results else 0,
        'median_precision': np.median([result.precision for result in valid_results]) if valid_results else 0,
        'median_recall': np.median([result.recall for result in valid_results]) if valid_results else 0,
        'median_f1_score': np.median([result.f1_score for result in valid_results]) if valid_results else 0,
        
        # Coverage metrics
        'statement_coverage': [result.coverage.get('statements', {}).get('percentage', 0) 
                               if isinstance(result.coverage, dict) else 0 for result in results],
        'branch_coverage': [result.coverage.get('branches', {}).get('percentage', 0)
                           if isinstance(result.coverage, dict) else 0 for result in results],
        
        # ATG Coverage metrics
        'atg_statement_coverage': [result.atg_coverage.get('statements', {}).get('percentage', 0) 
                                  if isinstance(result.atg_coverage, dict) else 0 for result in results],
        'atg_branch_coverage': [result.atg_coverage.get('branches', {}).get('percentage', 0)
                               if isinstance(result.atg_coverage, dict) else 0 for result in results],
        
        # Fallback metrics
        'failed_generation_rate': [result.failed_generation_rate for result in results],
        'test_failure_feedback_rate': [result.test_failure_feedback_rate for result in results],
        'error_correction_needed_rate': [result.error_correction_needed_rate for result in results],
        'partial_test_rate': [result.partial_test_rate for result in results],
        'individual_generation_rate': [result.individual_generation_rate for result in results],
        'found_no_allowed_identifiers_rate': [result.found_no_allowed_identifiers_rate for result in results],
        'schema_exceeded_size_rate': [result.schema_exceeded_size_rate for result in results],
        'no_atg_examples_rate': [result.no_atg_examples_rate for result in results],
        'used_code_context_fallback_rate': [result.used_code_context_fallback_rate for result in results],
        'used_atg_identifier_fallback_rate': [result.used_atg_identifier_fallback_rate for result in results],
        'exception_rate': [result.exception_rate for result in results],
        
        # Cost metrics
        'total_generation_cost': sum(result.total_generation_cost for result in results),
        'total_verification_cost': sum(result.total_verification_cost for result in results),
    ...
    if metrics['micro_precision'] + metrics['micro_recall'] > 0:
        metrics['micro_f1_score'] = 2 * (metrics['micro_precision'] * metrics['micro_recall']) / (metrics['micro_precision'] + metrics['micro_recall'])
    else:
...
def create_histogram(data: List[float], title: str, xlabel: str, figsize=(6, 4)) -> str:
    ...
    buf = BytesIO()
    plt.savefig(buf, format='png', dpi=90, bbox_inches='tight')  # Reduced DPI from 100 to 90
    plt.close()
...
def create_html_report(results: List[EvaluationResult], metrics: Dict[str, Any]) -> str:
    ...
    """
    main_metrics_section += create_histogram(metrics['precision'], 'Precision Distribution', 'Precision')
    main_metrics_section += """
                </div>
    ...
    """
    main_metrics_section += create_histogram(metrics['recall'], 'Recall Distribution', 'Recall')
    main_metrics_section += """
                </div>
    ...
    """
    main_metrics_section += create_histogram(metrics['f1_score'], 'F1 Score Distribution', 'F1 Score')
    main_metrics_section += """
                </div>
            </div>
        </div>
    ...
    fallback_metrics = [
        ...
        ('Found No Allowed Identifiers Rate', metrics['found_no_allowed_identifiers_rate']),
        ...
        ('Used ATG Identifier Fallback Rate', metrics['used_atg_identifier_fallback_rate']),
    ...
    for result in results:
        ...
        if isinstance(result.atg_coverage, dict) and 'statements' in result.atg_coverage:
            atg_stmt_cov = result.atg_coverage['statements'].get('percentage', 0)
            atg_branch_cov = result.atg_coverage.get('branches', {}).get('percentage', 0)
        ...
        fallback_items = [
            ...
            ('Found No Allowed Identifiers Rate', result.found_no_allowed_identifiers_rate),
            ...
            ('Used ATG Identifier Fallback Rate', result.used_atg_identifier_fallback_rate),
    ...
    return html_head + summary_section + main_metrics_section + coverage_section + fallback_section + individual_results_section + js_section + html_footer
