...
def calculate_aggregated_metrics(results: List[EvaluationResult]) -> Dict[str, Dict]:
    ...
    metrics = {
        ...
        "micro_precision": (
            total_verified_tests / total_generated_tests
            if total_generated_tests > 0
            else 0
        ),
        "micro_recall": (
            total_verified_tests / total_requirements if total_requirements > 0 else 0
        ),
        # Average metrics (macro-averages)
        "avg_precision": (
            np.mean([result.precision for result in valid_results])
            if valid_results
            else 0
        ),
        "avg_recall": (
            np.mean([result.recall for result in valid_results]) if valid_results else 0
        ),
        "avg_f1_score": (
            np.mean([result.f1_score for result in valid_results])
            if valid_results
            else 0
        ),
        "median_precision": (
            np.median([result.precision for result in valid_results])
            if valid_results
            else 0
        ),
        "median_recall": (
            np.median([result.recall for result in valid_results])
            if valid_results
            else 0
        ),
        "median_f1_score": (
            np.median([result.f1_score for result in valid_results])
            if valid_results
            else 0
        ),
