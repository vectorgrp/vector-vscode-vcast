...
def find_usable_environments(
    env_pattern=None, env_file=None, output_dir=None, get_coverage=False, percentile=90
):
    ...
    if get_coverage:
        ...
        if valid_coverages:
            ...
            big_envs = [
                env
                for env, coverage in coverage_data.items()
            ...
            with open(big_envs_path, 'w') as f:
                f.write('\n'.join(big_envs))
            print(
                f'Saved {len(big_envs)} large environments (>{percentile}th percentile) to {big_envs_path}'
            )
...
def main():
    parser = argparse.ArgumentParser(
        description='Prepare and analyze environment files for testing'
    )
    input_group = parser.add_mutually_exclusive_group(required=True)
    input_group.add_argument(
        '--env-pattern', help='Glob pattern to find environment files'
    )
    input_group.add_argument(
        '--env-file', help='File containing environment file paths (one per line)'
    )
    parser.add_argument(
        '--output-dir', required=True, help='Output directory for analysis results'
    )
    parser.add_argument(
        '--with-coverage', action='store_true', help='Perform coverage analysis'
    )
    parser.add_argument(
        '--percentile', type=float, default=90, help='Percentile for large environments'
    )
