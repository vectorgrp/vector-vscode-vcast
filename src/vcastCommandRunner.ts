...
import { processCommandOutput, type statusMessageType } from "./utilities";
...
export function executeVPythonScript(
  ...
  const returnData: commandStatusType = { errorCode: 0, stdout: "" };
  if (commandToRun) {
...
export function getJsonDataFromTestInterface(
  ...
  const jsonText = executeVPythonScript(commandToRun, enviroPath).stdout;
  try {
...
function processExceptionFromExecuteCommand(
  ...
  const commandStatus: commandStatusType = { errorCode: 0, stdout: "" };
...
export function executeCommandSync(
  ...
  try {
    ...
    commandStatus.stdout = execSync(commandToRun, { cwd }).toString().trim();
  } catch (error: any) {
...
export function executeWithRealTimeEcho(
  ...
  const clicast = spawn(command, argumentList, { cwd: CWD });
  ...
  clicast.on("exit", function (code: any) {
    vectorMessage("-".repeat(100));
    vectorMessage(
      `clicast: '${argumentList.join(" ")}' returned exit code: ${code.toString()}`
    );
    vectorMessage("-".repeat(100));
    if (callback) {
...
export function executeClicastWithProgress(
  ...
  vectorMessage(`Executing command: ${commandAndArguments.join(" ")}`);
  let commandStatus: commandStatusType = { errorCode: 0, stdout: "" };

  const cwd = path.dirname(testScriptPath);
  const command = commandAndArguments[0];
  const arguments_ = commandAndArguments.slice(1, commandAndArguments.length);
  ...
  vscode.window.withProgress(
    ...
    async (progress) => {
      return new Promise(async (resolve, reject) => {
        ...
        const commandHandle = spawn(command, arguments_, { cwd, shell: true });
