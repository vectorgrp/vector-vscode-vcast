...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const codedCheckbox = document.getElementById("enableCodedTestsCheckbox");

  // If extension injected a default, apply it
  ...
  function updateSuggestions(showAll = false) {
    ...
    filtered = showAll || !q ? [...compilers] : compilers.filter(c => c.toLowerCase().includes(q));
    activeIndex = -1;
    renderSuggestions();
  }
  ...
  // submit: include enableCodedTests boolean
