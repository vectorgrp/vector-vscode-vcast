 ...
 *
 * Heuristic: file is gone, parent directory is the OS temp dir
 * (`os.tmpdir()` matches Python's `tempfile.gettempdir()` on every platform
 * we ship to), and the basename matches `tempfile.NamedTemporaryFile`'s
 * default `tmp<random>` prefix with a `.csv` suffix.
 */
function looksLikeOrphanTempCsv(csvPath: string): boolean {
...
export function readRGWBundle(enviroPath: string): RGWBundle | null {
  ...
  if (fs.existsSync(files.origin)) {
    ...
    // write origin.json: if the configured source CSV is gone *and* the
    // path looks like a Python tempfile, the RGW was almost certainly
    // generated by us (genuine external imports point at persistent files).
    const csvPath = readCsvSourcePath(gatewayPath);
    if (csvPath && looksLikeOrphanTempCsv(csvPath)) {
