...
from design_decomposition.strategy.characteristic import CharacteristicDecompositionStrategy
...
class DesignDecomposer:
    ...
    def decompose_design(self, func_def, strategy=DecompositionStrategy.TEST_CASES, n=1, corrections=0):
        ...
        decomposition_results, messages = strategy.decompose(func_def, n=n, return_messages=True)
        
        # Move aggregation and error correction steps outside the strategies
        if n > 1:
            decomposition_result = self._combine_requirements(func_def, decomposition_results)
        else:
            decomposition_result = decomposition_results[0]

        decomposition_result = self._error_correction(func_def, decomposition_result, messages, corrections)
...
"""
    ...
    def _error_correction(self, func_def, decomposition_result, messages, max_corrections):
        iteration = 0
        ...
        while iteration < max_corrections:
            ...
            problems = self._find_problems(func_def, decomposition_result, history=error_correction_history)
...
"""
        ...
        history.append({
            "role": "assistant",
            "content": completion.choices[0].message.parsed.model_dump_json(indent=4)
        })
